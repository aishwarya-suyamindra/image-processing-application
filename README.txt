...
In our design, the controller ImageCommandController is the first to process the input provided. It maintains a map of the supported operations. The controller processes the command by tokenizing the input and takes the appropriate action. It takes in an object of the view and the model to work with, so it acts as a link between the view and the model. It calls the appropriate methods on the model and instructs the view on when to present information to the user.
The model, ImageProcessorImpl contains only the application logic. It does not provide or maintain any information regarding the display of the data to the user. It represents a set of operations that can be performed on an image. It is not coupled to the controller or the view, so it has no information about how its being used.
...
DESIGN CHANGES/ MODIFICATIONS (TO SUPPORT GUI):
  ...
  1. ImageGUICommandController.java - A new controller is added which manages the new GUI. As new controller is added rather than using existing one, as the events raised by graphical view is different from the ones raised in text based interaction. It is the first to respond to the events raised on the graphical view.
  ...
  3. To support the abstraction of the map of supported commands, the lambda functions that create the command objects are modified to split on newline, rather than whitespace.
...
The ImageGUICommandController contains an inner class FeaturesImpl that implements the Features interface, which contains the functionality to respond to the events on the view. It contains a map to transform the commands sent from the view to the existing command structure. In the process method of the controller, this features object is registered as the delegate on the view and the view is made visible. The controller itself is not made to implement the Features interface in order to avoid the view having access to the controller. Implementing it as an inner class ensures that the view only has limited access to the methods provided in the features interface, rather than all the public methods on the controller.
...
DESIGN CHANGES/ MODIFICATIONS (TO SUPPORT GUI):
  1. ImageProcessorRO.java - A new interface is added, which contains the operations that only give read only view of the model.
  2. ImageProcessROImpl.java - A new class  is added, which implements the ImageProcessorRO interface. It implements the necessary operations by using the existing model of type ImageProcessor. It takes a ImageProcessor model in its constructor and delegates the execution of the functionality to the model.
...
DESIGN CHANGES/ MODIFICATIONS (TO SUPPORT GUI):
  1. ImageHelperFactory.java - minimal change was made to update the error message thrown.
  2. ViewMessageType.java - a new enum is added, which represents the type of the message to be displayed by the view, so the view can then show it in an appropriate manner.
  3. CustomImage.java - Two new public methods and a private helper method are added to support the display of the histogram on the view. These methods return information about pixel frequency.
...
The graphical view works with a view model of type ImageProcessorRO. When an action is performed, the view notifies the delegate object and waits for its response. When the update method of the view is triggered, the view updates itself by fetching the appropriate image to display using the RO model and updates its histogram panel as well.
...
Image: https://pixabay.com/photos/hedgehog-cute-animal-little-nature-1215140/


The images in the resources folder are also downloaded from pixabay. The images have been resized.

https://pixabay.com/vectors/information-info-tips-icon-1481584/
https://pixabay.com/vectors/false-error-is-missing-absent-x-2061132/
https://pixabay.com/vectors/icon-symbol-confirmation-hook-ok-803718/
https://pixabay.com/vectors/upload-files-icon-1118929/
